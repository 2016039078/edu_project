...
public class ConController {
	...
	public String do_rooms_update_reservation_act(@ModelAttribute("conVO") ConVO vo,
			@ModelAttribute("roomsReservationVO") RoomsReservationVO vo2, ModelMap model, HttpSession session)
			throws Exception {
		...
		String r_id = vo2.getR_id();

		MessengerVO mvo = new MessengerVO();
		mvo.setR_id(r_id);

		String ms_id = messengerService.getMs_id(mvo);
		mvo.setMs_id(ms_id);

		Jeogi_userVO uvo = (Jeogi_userVO) session.getAttribute("userSession");
		mvo.setUser_id(uvo.getUser_id());

		messengerService.insert(mvo);
	...
	public String do_rooms_hosting_update_act(@ModelAttribute("conVO") ConVO vo, @ModelAttribute("jeogiVO") JeogiVO vo2,
			@ModelAttribute("precautionsVO") PrecautionsVO vo3, @ModelAttribute("reviewVO") ReviewVO vo4,
			@ModelAttribute("jeogi_userVO") Jeogi_userVO vo5,
			@ModelAttribute("roomsReservationVO") RoomsReservationVO vo6, ModelMap model, HttpServletRequest request,
		...
		try {
			...
			int delcnt = roomsReservationService.lastdatedelete(vo6);
			...
			for (int j = 0; j < datearry.length; j++) {
				...
				if (rreservedcnt >= 1) {
					...
					int hostinguproomreser = roomsReservationService.hostupdate(vo6);
	...
	public String do_review_insert_act(@ModelAttribute("conVO") ConVO vo, @ModelAttribute("jeogiVO") JeogiVO vo2,
			...
			@ModelAttribute("roomsReservationVO") RoomsReservationVO vo6, ModelMap model, HttpSession session) throws Exception {
	...
	public String do_review_delete_act(@ModelAttribute("reviewVO") ReviewVO vo,
			@ModelAttribute("jeogi_userVO") Jeogi_userVO vo2, HttpServletRequest request, ModelMap model, HttpSession session)
			throws Exception {
	...
	public String do_interest_add_act(@ModelAttribute("roomsVO") JeogiVO vo, ModelMap model, HttpServletRequest request)
			throws Exception {
	...
	@RequestMapping(value = "/reservation_select.do")
	public String do_sample_select(@ModelAttribute("roomsReservationVO") RoomsReservationVO vo, ModelMap model)
			throws Exception {

		LOGGER.debug("--------------------------------//reservation_select.jsp.do");
		LOGGER.debug(vo.toString());
		...
		List<?> reservedlist = roomsReservationService.reservedselect(vo);
		model.addAttribute("reservedlist", reservedlist);

		List<?> lastreservedlist = roomsReservationService.lastreservedselect(vo);
		model.addAttribute("lastreservedlist", lastreservedlist);

		return "jeogi/reservation_select";
	}
}
